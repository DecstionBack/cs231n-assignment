...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in xrange(hidden_layer - 1):
        ...
        #这里str(i+2)是为了保持gamma、beta和W、b的下标一致才写的，因为存储在字典中，所以下标没必要从0开始
  ...
  def loss(self, X, y=None):
    ...
    #Affine-->Batch Normalization-->Relu-->Dropput-->Affine
    X = X.reshape(X.shape[0],-1)
    for i in xrange(hidden_layer):
        #1. Get w and b from self.params
        ...
        if (self.use_batchnorm and (i > 0)):
            ...
            a_i, cache_i = affine_batch_relu_forward(X, w, b, gamma, beta, self.bn_params[i - 1])
            cache_history.append(cache_i)
        else:
            a_i, cache_i = affine_relu_forward(X, w, b)
            cache_history.append(cache_i)
            
        #Use dropout or not, dropout每一层的概率p都是一样的
        if self.use_dropout:
            a_i, drop_cache = dropout_forward(a_i, self.dropout_param)
            #和cache_history分开保存，否则反向求导下标不好写
            dropcache_history.append(drop_cache)
    ...
    ############################################################################
    #pass
    #y在函数调用的时候会传过来，如果不传入y则只是test，不需要计算loss和grads，dscores是对传入的scores求导
    ...
    for i in range(hidden_layer, 0, -1):
        #求导时先判断是否使用了dropout
        if self.use_dropout:
            dout = dropout_backward(dout, dropcache_history[i-1])
        
        #这儿的i的限制是因为第一层输入层后面没有加入batch_normalization层，前面不用做归一化，第一层后面直接batch normalization就可以了
        if (self.use_batchnorm) and (i>1):
            da, dw, db, dgamma, dbeta = affine_batch_relu_backward(dout, cache_history[i-1])
            grads['gamma'+str(i-1)] = dgamma
            grads['beta'+str(i-1)] = dbeta
            #if (i>4):
            #    print 'i=%s'%i
            #    print grads['gamma4'].shape
        else:
